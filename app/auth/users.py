...
def adjust_timezone(date_string):
    ...
    if len(parts[1]) == 1:  # Check if the minutes in the timezone are not two digits
        return f"{parts[0]}:0{parts[1]}"  # Pad the minute with a zero
    return date_string
...
def find_optimal_slots(access_token):
    ...
    local_timezone = pytz.timezone('America/New_York')  # Adjust this to the timezone of your choice
    start_of_day = local_timezone.localize(datetime.strptime(f"{today} 09:00", '%Y-%m-%d %H:%M'))
    end_of_day = local_timezone.localize(datetime.strptime(f"{today} 17:00", '%Y-%m-%d %H:%M'))
    slots = [(start_of_day, end_of_day)]

    for event in events:
        start = datetime.fromisoformat(adjust_timezone(event['start']['dateTime'][:-1])).astimezone(local_timezone)
        end = datetime.fromisoformat(adjust_timezone(event['end']['dateTime'][:-1])).astimezone(local_timezone)
        new_slots = []
